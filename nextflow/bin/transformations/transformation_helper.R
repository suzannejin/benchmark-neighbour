...
make_knn_graph <- function(transformation, dat, pca_dim, k_nearest_neighbors){
  if(transformation ==  "sanity_dists"){
    ...
    if(pca_dim >= nrow(dat) || pca_dim >= ncol(dat)){ 
      ...
      red_dat <- BiocSingular::runPCA(dat, rank = pca_dim, get.rotation = FALSE, BSPARAM = BiocSingular::ExactParam())$x
    }else{
      red_dat <- BiocSingular::runPCA(dat, rank = pca_dim, get.rotation = FALSE, BSPARAM = BiocSingular::FastAutoParam())$x
    }
    # compute neighbours
    BiocNeighbors::findAnnoy(red_dat, k = k_nearest_neighbors, warn.ties = FALSE)$index
  }
