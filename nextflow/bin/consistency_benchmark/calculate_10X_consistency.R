...
pa <- argparser::add_argument(pa, "--data_id", type = "character", help = "The dataset id") 
...
KNNs <- readRDS(file.path(pa$working_dir, "knn/", paste0(pa$data_id, '.rds')))
...
write_tsv(res, file.path(pa$working_dir, "metric/", paste0(pa$data_id, '.tsv')))